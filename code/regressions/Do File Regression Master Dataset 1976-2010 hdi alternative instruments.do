...
use "C:\Users\Alex\Desktop\Master Dataset 16.07.2016 (1976-2010) hdi alternative instruments.dta"

encode country, gen(cntry)
xtset cntry period

quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen1 lagpredictedopen1)  if MRW==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg opendots predictedopen1 lagpredictedopen1 lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)

drop if sample!=1
pcorr2 opendots predictedopen1 lagpredictedopen1 lnpop largedisaster laglargedisaster
...
quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen1 lagpredictedopen1)  if MRWI==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg opendots predictedopen1 lagpredictedopen1 lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)
...
quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen1) if Full==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg opendots predictedopen1 lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)

drop if sample!=1
pcorr2 opendots predictedopen1 lnpop largedisaster laglargedisaster

drop sample

esttab using First_Stage_1976-2010_hdi_opendots_alternative_instruments.tex, replace compress se r2 ar2 star(* 0.1 ** 0.05 *** 0.01) ///
...
xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen1 lagpredictedopen1) if MRW==1, fe first savefirst small cluster(cntry)
...
xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen1 lagpredictedopen1) if MRWI==1, fe first savefirst small cluster(cntry)
...
xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen1) if Full==1, fe first savefirst small cluster(cntry)
...
eststo: quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen1 lagpredictedopen1) if MRW==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg hdi opendots lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)
drop sample

eststo: quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen1 lagpredictedopen1) if MRWI==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg hdi opendots lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)
drop sample

eststo: quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen1) if Full==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg hdi opendots lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)
drop sample

esttab using Second_Stage_1976-2010_hdi_opendots_alternative_instruments.tex, replace compress se r2 ar2 star(* 0.1 ** 0.05 *** 0.01) ///
...
quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen2 lagpredictedopen2)  if MRW==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg opendots predictedopen2 lagpredictedopen2 lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)

drop if sample!=1
pcorr2 opendots predictedopen2 lagpredictedopen2 lnpop largedisaster laglargedisaster
...
quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen2 lagpredictedopen2)  if MRWI==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg opendots predictedopen2 lagpredictedopen2 lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)
...
quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen2) if Full==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg opendots predictedopen2 lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)

drop if sample!=1
pcorr2 opendots predictedopen2 lnpop largedisaster laglargedisaster
...
xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen2 lagpredictedopen2) if MRW==1, fe first savefirst small cluster(cntry)
...
xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen2 lagpredictedopen2) if MRWI==1, fe first savefirst small cluster(cntry)
...
xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen2) if Full==1, fe first savefirst small cluster(cntry)
...
eststo: quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen2 lagpredictedopen2) if MRW==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg hdi opendots lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)
drop sample

eststo: quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen2 lagpredictedopen2) if MRWI==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg hdi opendots lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)
drop sample

eststo: quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen2) if Full==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg hdi opendots lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)
drop sample
...
quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen3 lagpredictedopen3)  if MRW==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg opendots predictedopen3 lagpredictedopen3 lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)

drop if sample!=1
pcorr2 opendots predictedopen3 lagpredictedopen3 lnpop largedisaster laglargedisaster
...
quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen3 lagpredictedopen3)  if MRWI==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg opendots predictedopen3 lagpredictedopen3 lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)
...
quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen3) if Full==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg opendots predictedopen3 lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)

drop if sample!=1
pcorr2 opendots predictedopen3 lnpop largedisaster laglargedisaster
...
xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen3 lagpredictedopen3) if MRW==1, fe first savefirst small cluster(cntry)
...
xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen3 lagpredictedopen3) if MRWI==1, fe first savefirst small cluster(cntry)
...
xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen3) if Full==1, fe first savefirst small cluster(cntry)
...
eststo: quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen3 lagpredictedopen3) if MRW==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg hdi opendots lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)
drop sample

eststo: quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen3 lagpredictedopen3) if MRWI==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg hdi opendots lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)
drop sample

eststo: quietly xi: xtivreg2 hdi lnpop largedisaster laglargedisaster i.period (opendots = predictedopen3) if Full==1, fe first savefirst small cluster(cntry)
generate sample = e(sample)
eststo: quietly xi: xtreg hdi opendots lnpop largedisaster laglargedisaster i.period if sample==1, fe vce(cluster cntry)
drop sample
